<template>
  <AuthenticatedLayout title="Rent Collections Detail">
    <div class="p-6 space-y-5">

      <!-- Header -->
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 class="text-xl font-bold fv-text-primary">Rent Collections Detail</h1>
          <p class="text-xs fv-text-muted mt-1">All rent collection rows for a selected period</p>
        </div>
      ...
      <div v-if="result">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div class="fv-card p-4 text-center" style="border-left:3px solid #34d399;">
            ...
            <p class="text-lg font-bold mt-1" style="color:#6ee7b7;">{{ fmt(result.total_collected) }}</p>
          </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr>
                ...
                <th class="fv-th text-left">Unit Type</th>
            ...
            <tbody>
              <tr v-for="row in result.rows" :key="row.id" class="fv-tr">
                <td class="fv-td whitespace-nowrap">{{ row.collection_date }}</td>
                <td class="fv-td font-medium fv-text-primary">{{ row.unit_label }}</td>
                <td class="fv-td fv-text-muted capitalize">{{ row.property_nature }}</td>
                <td class="fv-td fv-text-primary">{{ row.tenant_name }}</td>
                ...
                <td class="fv-td text-right font-semibold fv-text-primary whitespace-nowrap">
                  {{ fmt(row.collection_amount) }} {{ row.currency }}
                </td>
                <td class="fv-td">
                  <span class="fv-badge" :style="statusStyle(row.status)">{{ row.status }}</span>
                </td>
                <td class="fv-td fv-text-muted">{{ row.collected_date ?? '—' }}</td>
...
async function runReport() {
  ...
  try {
    const url = route('company.reports.rent-collections.data', { company: props.company.id })
