<template>
  <AuthenticatedLayout title="Properties Rent Collection">
    <div class="p-6 space-y-6">

      <!-- ── Page Header ─────────────────────────────────────────── -->
      <div>
        <h1 class="text-lg font-bold fv-text-primary">Properties Rent Collection</h1>
        <p class="text-xs fv-text-muted mt-0.5">Outstanding rent collections across every property — pending &amp; overdue only.</p>
      </div>
      ...
      <div class="fv-card !p-4 flex flex-wrap items-end gap-3">
        ...
        <div>
          <label class="block text-xs fv-text-label mb-1">Tenant</label>
          <input v-model="filters.tenant" @keyup.enter="fetchData(1)" type="text" placeholder="Search tenant name…"
            class="fv-input rounded-lg px-3 py-2 text-sm" style="min-width:220px" />
        </div>
        <button @click="fetchData(1)" class="fv-btn-secondary text-sm font-semibold px-4 py-2 rounded-lg">Search</button>
        <button v-if="filters.property_id || filters.tenant" @click="clearFilters" class="fv-btn-secondary text-sm px-4 py-2 rounded-lg">
          Clear
        </button>
      ...
      <div class="fv-card !p-0 overflow-hidden">
        ...
        <div v-else-if="!rows.length" class="text-center py-12 fv-text-muted text-sm">
          No outstanding rent collections found.
        </div>
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b fv-divider">
                <th class="text-left py-2.5 px-4 text-xs fv-text-label font-semibold">Property / Unit</th>
                <th class="text-left py-2.5 px-4 text-xs fv-text-label font-semibold">Tenant</th>
                <th class="text-left py-2.5 px-4 text-xs fv-text-label font-semibold">Period</th>
                <th class="text-left py-2.5 px-4 text-xs fv-text-label font-semibold">Collection Date</th>
                ...
                <th class="text-left py-2.5 px-4 text-xs fv-text-label font-semibold">Apply Collection</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in rows" :key="row.id" class="border-b fv-divider">
                <td class="py-2.5 px-4 fv-text-primary">
                  {{ row.property_name }}
                  <span v-if="row.unit_label && row.unit_label !== row.property_name" class="fv-text-muted text-xs block">{{ row.unit_label }}</span>
                </td>
                <td class="py-2.5 px-4 fv-text-muted">{{ row.tenant_name }}</td>
                <td class="py-2.5 px-4 fv-text-muted text-xs">{{ formatDate(row.period_from) }} – {{ formatDate(row.period_to) }}</td>
                <td class="py-2.5 px-4 fv-text-primary">{{ formatDate(row.collection_date) }}</td>
                <td class="py-2.5 px-4 text-right font-medium fv-text-primary">
                  {{ fmt(row.collection_amount) }} <span class="text-xs fv-text-muted">{{ row.currency }}</span>
                </td>
                ...
                <td class="py-2.5 px-4">
                  <div class="flex items-center gap-2">
                    <input type="date" v-model="collectDates[row.id]" class="fv-input rounded-lg px-2 py-1.5 text-xs" style="width:9.5rem;" />
                    <button @click="collect(row)" :disabled="collectingId === row.id"
                      class="fv-btn-gold text-xs font-semibold px-3 py-1.5 rounded-lg">
                      {{ collectingId === row.id ? '…' : 'Collect' }}
                    </button>
                  </div>
                </td>
              </tr>
...
const collectDates  = ref({})   // row.id → date string, defaults to today
const collectingId  = ref(null)
const pagination = ref({ current_page: 1, per_page: 25, total: 0, last_page: 1 })

const filters = ref({ property_id: '', tenant: '' })
...
const exportUrl = computed(() => {
  ...
  if (filters.value.tenant) params.set('tenant', filters.value.tenant)
  const qs = params.toString()
  return route('company.properties.rent-collections.export', props.company.id) + (qs ? `?${qs}` : '')
})
...
async function fetchData(page = 1) {
  ...
  try {
    const { data } = await axios.get(
      route('company.properties.rent-collections.data', props.company.id),
      { params: {
    ...
    for (const row of rows.value) {
      if (!(row.id in collectDates.value)) collectDates.value[row.id] = today
    }
  } catch (e) {
    console.error(e)
  } finally {
...
function clearFilters() {
  filters.value = { property_id: '', tenant: '' }
  fetchData(1)
}

// markCollected (RentContractController) redirects with a flash message
// rather than returning JSON, so this goes through Inertia's router
// (same pattern as Contracts/Show.vue) rather than axios — then we
// refetch this page's own data so the row disappears and the bucket
// totals stay correct.
function collect(row) {
  collectingId.value = row.id
  router.patch(
    route('company.properties.contracts.collections.collected', [props.company.id, row.property_id, row.rent_contract_id, row.id]),
    { collected_date: collectDates.value[row.id] || today },
    {
      preserveScroll: true,
      onSuccess: () => fetchData(pagination.value.current_page),
      onError: () => alert('Could not mark this collection as collected.'),
