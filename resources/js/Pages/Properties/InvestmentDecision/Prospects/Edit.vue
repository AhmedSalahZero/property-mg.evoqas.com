...
const form = useForm({
  prospect_name:         props.prospect.prospect_name,
  status:                props.prospect.status,
  nature:                props.prospect.nature ?? 'unit',
  country:               props.prospect.country ?? 'Egypt',
  governorate:           props.prospect.governorate ?? '',
  province:              props.prospect.province ?? '',
  location:              props.prospect.location ?? '',
  currency:              props.prospect.currency ?? 'EGP',
  notes:                 props.prospect.notes ?? '',
  property_category_id:  props.prospect.property_category_id ?? '',
  property_type_id:      props.prospect.property_type_id ?? '',
  area:                  props.prospect.area ?? '',
  unit_of_measurement:   props.prospect.unit_of_measurement ?? '',
  purchase_price:        props.prospect.purchase_price ?? '',
  expected_monthly_rent: props.prospect.expected_monthly_rent ?? '',
  units: (props.prospect.units ?? []).map(u => ({
...
function submit() {
  ...
  form.put(route('company.properties.investment-decision.update', [props.company.id, props.prospect.id]))
}
</script>

<template>
  <Head title="Edit Investment Prospect" />
  <AuthenticatedLayout>
    <div class="max-w-6xl mx-auto py-8 px-4">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-xl font-bold fv-text-primary">Edit Prospect</h1>
          <p class="fv-text-muted text-sm mt-1">{{ prospect.prospect_name }}</p>
        </div>
      ...
      <form @submit.prevent="submit" class="fv-card rounded-xl p-6 space-y-5">
        <div>
          ...
          <input v-model="form.prospect_name" type="text" class="fv-input w-full rounded-lg px-3 py-2 text-sm" />
        ...
        <div>
          ...
          <p class="fv-text-muted text-xs mt-2" v-else-if="prospect.nature && prospect.nature !== 'unit'">
            Switching to Single Unit will discard the units currently attached to this prospect once you save.
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="form.location" type="text" class="fv-input w-full rounded-lg px-3 py-2 text-sm" />
        </div>
        ...
        <template v-if="!isMultiUnit()">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.unit_of_measurement" type="text" class="fv-input w-full rounded-lg px-3 py-2 text-sm" />
            </div>
          

          <div>
            <label class="fv-label">Purchase Price *</label>
            <input v-model="form.purchase_price" type="number" min="0" step="0.01" class="fv-input w-full rounded-lg px-3 py-2 text-sm" />
          ...
          <div>
            ...
            <input v-model="form.expected_monthly_rent" type="number" min="0" step="0.01" class="fv-input w-full rounded-lg px-3 py-2 text-sm" />
          </div>
          </div>
        </template>
          ...
          <div v-for="(unit, idx) in form.units" :key="idx" class="p-4 rounded-lg space-y-3" style="background:var(--fv-bg); border:1px solid var(--fv-border);">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input v-model="unit.unit_of_measurement" type="text" class="fv-input w-full rounded-lg px-2 py-1.5 text-xs" />
              </div>
            </div>
