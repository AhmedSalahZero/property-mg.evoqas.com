<template>
  <AuthenticatedLayout :title="`Due Installments — ${property.property_name}`">
    <div class="p-6 space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg font-bold fv-text-primary">Due Installments</h1>
      ...
      <template v-else>
        <div class="fv-card !p-0 overflow-hidden">
          ...
          <div class="flex border-b fv-divider px-6">
            <button
          ...
          <div class="px-6 py-5">
            ...
            <div v-if="activeTab === 'plan'">
              ...
              <div class="grid grid-cols-2 gap-4 mb-5" style="max-width:600px">
                <div>
              ...
              <template v-if="form.installment_type === 'regular'">
                ...
                <!-- Signing + Reservation -->
                <div class="grid grid-cols-4 gap-4 mb-5" style="max-width:900px">
                  <div>
            ...
            <div v-if="activeTab === 'schedule'">
              ...
              <template v-else>
                <!-- Summary strip -->
                <div class="grid grid-cols-3 gap-3 mb-5" style="max-width:700px">
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <td class="py-2 pr-4 text-center">
                          <span class="fv-badge" :class="statusClass(effectiveStatus(due))"
                            :style="effectiveStatus(due) === 'overdue' ? 'color:#f87171' : ''">{{ effectiveStatus(due) }}</span>
                        </td>
          ...
          <!-- ── Footer ────────────────────────────────── -->
          <div class="flex items-center justify-between px-6 py-4 border-t fv-divider">
            <Link :href="route('company.properties.index', company.id)" class="fv-btn-secondary text-sm font-medium px-5 py-2 rounded-lg">
              Back to Properties
            </Link>
            <button
              v-if="activeTab === 'plan'"
...
<script setup>
import { ref, computed, onMounted, h, Teleport } from 'vue'
...
async function loadPlan() {
    ...
    try {
        ...
        if (data.plan) {
            ...
            form.value = {
                ...
                variable_dues:       dues.value.length && p.installment_type === 'variable'
                    // due_date comes back from the backend as a full ISO
                    // timestamp string because PropertyInstallmentDue casts
                    // due_date as 'date'. The native <input type="date">
                    // requires exactly YYYY-MM-DD, hence .slice(0, 10).
                    // `id` is round-tripped here (and sent back on save) so
                    // the backend can recognize "this row, edited" instead
                    // of matching purely by date.
        ...
        // If a schedule already exists, the user is coming here to check/
        // manage payments, not to re-fill out the plan — land on Payment
        // Schedule directly. A brand-new property (no dues yet) still
        // opens on Installment Plan since there's nothing to show yet.
        activeTab.value = dues.value.length ? 'schedule' : 'plan'
    } catch (e) {
        console.error(e)
    } finally {
        loading.value = false
...
// Defense-in-depth (confirmed July 2026 session): same fix as
// Contracts/Show.vue's effectiveStatus() — the stored `status` column
// only flips 'pending' → 'overdue' via the daily property:mark-overdue
// command, which depends on a system cron that can easily go
// unconfigured on a dev machine. This computes the correct badge live
// from due_date instead of trusting that job to have run recently.
function effectiveStatus(due) {
    if (due.status === 'pending' && new Date(due.due_date) < new Date(new Date().toDateString())) {
        return 'overdue'
    }
    return due.status
}
